...
class DeliveryProofDelivery extends StatefulWidget {
  const DeliveryProofDelivery({super.key});

  @override
  State<DeliveryProofDelivery> createState() => _DeliveryProofDelivery();
}

class _DeliveryProofDelivery extends State<DeliveryProofDelivery> {
  dynamic image;
  XFile? fileImage;
  File? imageFile;
...
Future<void> uploadImage(File fileImage) async {
  try {
    ...
    final trackingId = riderParcelListD[booking_index]['tracking_id'];
    final riderId = riderParcelListD[booking_index]['rider_id'];
    ...
    if (querySnapshot.docs.isEmpty) {
      print('No documents matched the query for trackingId: $trackingId and riderId: $riderId');
      return;
    }

    // Update each document found
    for (QueryDocumentSnapshot doc in querySnapshot.docs) {
      // First, set the picture_url field to null to "delete" the existing image
      ...
      print('Picture URL updated successfully for document ${doc.id}');
    }
  } catch (e) {
 ...
 Future<void> completeDelivery() async {
  ...
  // for (var parcel in riderParcelList[booking_index]['tracking_id']) {
  //   await firestore.collection('parcel').doc(parcel['parcel_id']).update({
  //     'status': 'delivered',
  //   });
  //}

  // Update the rider status to 'idle'
  // String riderId = currentUserID;
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      Text(
                                        rider_parcel_list_delivered.isEmpty
                                            ? 'Loading...'
                                            : rider_parcel_list_delivered[
                                                        booking_index]
                                                   ['name'],
