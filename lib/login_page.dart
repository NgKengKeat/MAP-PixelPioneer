...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<bool> _emailError() async {
    ...
    var riderSnapshot = await firestore
        .collection('users')
        .where('email', isEqualTo: email)
        .where('rider_id', isNotEqualTo: null)
        .get();
  ...
  Future<bool> _passwordError() async {
    try {
      ...
      // The password is correct, return false (no error)
      return false;
    } catch (e) {
      // If sign-in fails, it will throw an exception
      // print("Error with password: $e");

      // There is an error with the password, return true
      return true; 
    }
  }
                      ...
                      child: Row(
                          ...
                          children: [
                            Container(
                              ...
                              height: 200,
                              child: const Image(
                                image: AssetImage('assets/images/cpp_logo.png'),
                              ),
                            ),
                          ]),
                    ),
